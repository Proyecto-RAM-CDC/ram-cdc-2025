...
interface IProps {
  className: string;
  data: ChartsData;
  setCurrentBarGeneroChartData: React.Dispatch<
    React.SetStateAction<ChartDataEntry[] | null>
  >;
  setCurrentChartMainTitle: React.Dispatch<React.SetStateAction<string | null>>;
  setCurrentChartSubTitle: React.Dispatch<React.SetStateAction<string | null>>;
  currentChartMainTitle: string;
  currentChartSubTitle: string;
  currentBarGeneroChartData: ChartDataEntry[];
}
...
const handleStateChange = (
  ...
  if (chartKey === "barChart" && chartType?.chartData) {
    setCurrentBarGeneroChartData(chartType.chartData[selectedState]);
  }

  if (chartMainTitles && chartSubTitles) {
    setCurrentChartMainTitle(chartMainTitles[selectedState]);
  }

  if (chartMainTitles && chartSubTitles) {
    setCurrentChartSubTitle(chartSubTitles[selectedState]);
  }
};

const BarChartGenero: React.FC<IProps> = (props) => {
  const data: ChartsData = props.data;
  const setCurrentBarGeneroChartData: React.Dispatch<
    React.SetStateAction<ChartDataEntry[] | null>
  > = props.setCurrentBarGeneroChartData;
  const setCurrentChartMainTitle: React.Dispatch<
    React.SetStateAction<string | null>
  > = props.setCurrentChartMainTitle;
  const setCurrentChartSubTitle: React.Dispatch<
    React.SetStateAction<string | null>
  > = props.setCurrentChartSubTitle;
  ...
  return (
    ...
    >
        ...
        <ul className="dropdown-content menu menu-xs bg-secondary text-secondary-content rounded-box z-[1] w-52 p-1 shadow max-h-60 overflow-y-auto">
          {
            ...
            data.barChart?.chartData &&
              Object.keys(data.barChart.chartData)
                .sort()
                .map((key) => (
                  <li key={key}>
                    <button
                      onClick={() =>
                        handleStateChange(
                          data,
                          setCurrentBarGeneroChartData,
                          setCurrentChartMainTitle,
                          setCurrentChartSubTitle,
                          "barChart",
                          key
                        )
                      }
      ...
      <h2 className="mt-1 text-center w-full text-xs sm:text-base font-semibold">
        {props.currentChartMainTitle}
      </h2>{" "}
      <h3 className="mb-4 text-center w-full text-xs sm:text-sm font-normal">
        {props.currentChartSubTitle}
      </h3>{" "}
      <ResponsiveContainer width="100%" height="100%">
        {data.barChart?.chartData && props.currentBarGeneroChartData ? (
          RenderBarChart(props.currentBarGeneroChartData)
        ) : (
